...
def test_auction_matches_scipy(device: str, num_rows: int, num_cols: int):
    """The auction reaches the optimal cost on well-conditioned problems of every shape."""
    ...
    assigned, solved = batched_auction(torch.as_tensor(cost, dtype=torch.float32, device=device), row_valid)
    assert bool(solved.all()), "auction failed to converge on a well-conditioned problem"
...
def test_auction_is_scale_invariant(device: str, scale: float):
    """Per-problem normalisation should make the solver indifferent to the cost scale."""
    rng = np.random.default_rng(7)
    cost = (rng.random((4, 30, 60)) - 0.5) * scale
    assigned, solved = batched_auction(torch.as_tensor(cost, dtype=torch.float32, device=device), torch.ones(4, 30, dtype=torch.bool, device=device))
...
@pytest.mark.parametrize("device", DEVICES)
def test_auction_handles_degenerate_costs(device: str):
    ...
    assigned, solved = batched_auction(torch.as_tensor(cost, dtype=torch.float32, device=device), torch.ones(8, 25, dtype=torch.bool, device=device))
    assert bool(solved.all())
...
@pytest.mark.parametrize("device", DEVICES)
def test_auction_respects_padding_and_forbidden_entries(device: str):
    ...
    assigned, solved = batched_auction(
...
@pytest.mark.parametrize("device", DEVICES)
def test_auction_rejects_infeasible_problems(device: str):
...
def test_matcher_device_solver_agrees_with_host(device: str, num_preds: int):
    ...
    dev = Matcher(default_solver="scipy", adaptive_solver=False, device_solver="auction")
