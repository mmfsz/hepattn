#!/bin/bash
# Per-rank launcher, executed by srun once per rank.
#
...
# NCCL. Give every rank its own compile cache on node-local disk instead.
...
# --frozen: use the on-disk env/lock as-is (no network, no lock race across ranks).
exec pixi run --frozen python "$@"
