...
# objective -- e.g. scoring the buggy-loss baseline under the v2 mask losses, which puts
# both models on one yardstick.
#
...
CLIC=/blue/avery/m.mazza/projects/fastml/hepattn/src/hepattn/experiments/clic
cd "$CLIC"
...
python3 - "$CONFIG_IN" "$CONFIG_RUN" "$OUTDIR" <<'EOF'
import re, sys
