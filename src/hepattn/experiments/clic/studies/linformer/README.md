...
The forward pass, with the **full model's** encoder shapes (batch `B`,
`n = m = 168` tokens, `dim = 256`, `heads = 16`, `dim_head = 16`, `k = 64`).
The arms run the small model, `dim 64` and 8 heads; only the sequence lengths,
168 and `k`, are shared:
...
### 4.3 `configs/linformer.yaml` — the ported config, on the FULL model

**This is not one of the study's arms.** It is Lindsey's configuration carried
over by the port, built on `base.yaml`: the **full 12 M paper model**, `dim 256`,
16 heads, Linformer in *both* the encoder and the decoder, masked attention off.
It is kept as the reference for what was ported and is what the CPU checks in
section 10 are run against. The arms that actually run are the 0.82 M small model
and live in `studies/linformer/configs/` — see section 10's table.
...
The multiply counts below are per unit of embedding size, so they hold for either
model. The **parameter** arithmetic does not: the compression matrices cost
`2 · seq_len · k` whatever `dim` is, which is why the same `k` is worth +8 % of an
attention block at `dim 256` and +128 % at `dim 64`. The arms run at `dim 64`.
...
**Ruled out: masking before the projection.** The first thing anyone proposes is
to apply the mask while the real tokens still exist, ahead of the projection.
...
| `src/hepattn/models/decoder.py` | `sa_attn_kwargs`, which is what lets Linformer sit on the query self-attention alone |
| `tests/models/test_linformer.py` | shapes, padding-invariance, refusals, weight persistence |
| `configs/linformer.yaml` | the ported config, **full 12 M model**, encoder *and* decoder, mask off. Reference only, never run |
...
Checked against `clic-paper-main` at `03110f1`. `tests/models/` — 96 passed.

**Two models appear below and they are not the same model.** The parameter counts
are only comparable within a block.

#### The full 12 M model (`configs/linformer.yaml`) — checked, never run

Not an arm. This is the ported config of 4.3, used here because it exercises
Linformer in the encoder *and* the decoder in one build.

- It differs from the current `base.yaml` by the intended changes only (name,
  2e-4 peak LR, `attn_type`, the two projection sizes, `mask_attention: false`);
  none of the later config work drifted past it.
- Building it and running one forward and backward pass on CPU: **12.452 M**
  parameters, all four decoder layers plus the final one, and **no parameter left
  without a gradient**. The last is the one that matters: the norms and
  value-residual mixes of 4.4 must all receive gradient, or DDP raises on the
  multi-GPU runs.
- `configs/base.yaml`, its baseline, is **12.065 M**, so compression costs
  +0.387 M (+3.2 %) *on the full model*. That is also the only comparison
  available on CPU: `base.yaml`'s `flash-varlen` backend has no CPU kernel, while
  Linformer runs anywhere.

#### The 0.82 M small model (`studies/linformer/configs/`) — the arms that run

Every submitted arm is generated from `configs/base_small.yaml`: `dim 64`, 8
heads. This is the FPGA-relevant model and the one all the numbers in section 11
refer to.

| arm | parameters |
| --- | --- |
| `clic_paper_small_phi_ref`, `clic_paper_small_no_mask_attn` | 0.820 M |
| `clic_paper_small_linformer_unmasked` | 1.026 M, no parameter without a gradient |

The 0.206 M difference is exactly `6 × 2×168×64` for the encoder plus
`4 × 2×150×64` for the decoder's query self-attention — the compression matrices
and nothing else, since the Linformer projections come out the same size as the
attention block they replace (16,640 either way at `dim 64`). So **+25 %
parameters for about −24 % attention multiplies**, and note how much harsher that
is than the +3.2 % the full model pays for the same `k`: section 5 says why.

The reference and step 0 cannot be forward-tested on CPU, so the phi-sorting path
was checked by overriding the reference to torch attention.
